...
import cn.itcast.tags.tools.{HBaseTools, ProfileTools, TagTools}
...
abstract class AbstractModel(modelName: String, modelType: ModelType) extends Logging {
  ...
  def mergeAndSaveTag(modelDF: DataFrame): Unit = {
    ...
    // 5.b. 将用户标签数据合并
    val newProfileDF: DataFrame = ProfileTools.mergeNewProfileTags(modelDF, profileDF, ids)
    // 5.c. 保存HBase表中
