...
object HBaseTools {
	...
	def read(spark: SparkSession, zks: String, port: String,
		...
		val datasRDD: RDD[(ImmutableBytesWritable, Result)] = spark.sparkContext
			.newAPIHadoopRDD(
				conf, //
	...
	def write(dataframe: DataFrame, zks: String, port: String,
		...
		val columns: Array[String] = dataframe.columns  // 获取DataFrame中所有列的名称
